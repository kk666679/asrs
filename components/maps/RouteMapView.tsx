...
const RouteMapView: React.FC<RouteMapViewProps> = ({
  ...
  const allPoints = useMemo(() => [origin, ...destinations], [origin, destinations]);
  ...
  useEffect(() => {
    if (allPoints.length > 0) {
      ...
      // Only update state when center actually changes to avoid update loops
      setMapCenter(prev => {
        const EPS = 1e-6;
        if (prev && Math.abs(prev.lat - avgLat) < EPS && Math.abs(prev.lng - avgLng) < EPS) {
          return prev;
        }
  ...
  // Build a simple GeoJSON FeatureCollection from points for the Fleet map
  const geoJson = useMemo(() => ({
    type: 'FeatureCollection',
    features: allPoints.map(p => ({
      type: 'Feature',
      properties: { id: p.id, name: p.name, priority: p.priority },
      ...
      <div className="lg:col-span-2">
        <Card>
          ...
          <CardContent>
            <FleetLocationMap
              geoJson={geoJson}
              points={allPoints.map(p => ({ id: p.id, name: p.name, lat: p.lat, lng: p.lng }))}
              center={mapCenter}
              zoom={10}
              height="500px"
            />
