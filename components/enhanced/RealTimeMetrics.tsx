...
export default function RealTimeMetrics() {
  ...
  // Accept live metric updates from WebSocket
  useWebSocketEvent<Partial<Metric> & { id: string }>('METRICS_UPDATE', (data) => {
    setMetrics(prev => prev.map(m => m.id === data.id ? { ...m, ...data } : m));
  });

  // Simulate updates when no WS data
  useEffect(() => {
    const interval = setInterval(() => {
      setMetrics(prev => prev.map(metric => {
        const delta = (Math.random() - 0.5) * metric.value * 0.02;
        const newValue = Math.max(0, metric.value + delta);
        return {
          ...metric,
          value: newValue,
          change: (delta / metric.value) * 100,
          trend: delta > 0.5 ? 'up' : delta < -0.5 ? 'down' : 'stable',
          status: metric.id === '3' && newValue > 1 ? 'critical'
                : metric.id === '2' && newValue < 80 ? 'warning'
                : 'good',
        };
      }));
    }, 3000);
