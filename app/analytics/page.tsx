...
import { EnhancedPageLayout, StatCard, statGridVariants } from '@/components/shared';
...
export default function AnalyticsPage() {
  ...
  if (loading) {
    return <EnhancedPageLayout title="Advanced Analytics" loading skeletonRows={4}><></></EnhancedPageLayout>;
  }
  ...
  return (
    <EnhancedPageLayout
      ...
      quickActions={[
        { label: 'Refresh', onClick: () => fetchAnalyticsData() },
        { label: 'Last 7 days', onClick: () => setPeriod('7d') },
        { label: 'Last 30 days', onClick: () => setPeriod('30d') },
        { label: 'Last 90 days', onClick: () => setPeriod('90d') },
      ]}
      headerRight={
        <>
          ...
          <Select value={period} onValueChange={setPeriod}>
            <SelectTrigger className="w-36 glass-effect neon-border text-blue-300">
              <SelectValue />
            </SelectTrigger>
    ...
    >
      ...
      <motion.div
        ...
        animate="show"
      >
        <StatCard label="Total Items" value={analyticsData?.summary.totalItems || 0} icon={<Activity className="h-5 w-5" />} color="blue" />
        <StatCard label="Inventory Turnover" value={analyticsData?.kpis.inventoryTurnover || 0} icon={<TrendingUp className="h-5 w-5" />} color="green" />
        <StatCard label="Space Utilization" value={`${analyticsData?.kpis.spaceUtilization || 0}%`} icon={<Activity className="h-5 w-5" />} color="cyan" />
        <StatCard label="Stock Alerts" value={analyticsData?.kpis.stockAlertsCount || 0} icon={<AlertTriangle className="h-5 w-5" />} color="red" />
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="glass-effect hover-glow">
          <CardHeader>
            <CardTitle className="text-glow">Movement Trends</CardTitle>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={analyticsData?.trends || []}>
                ...
                <Tooltip 
                  ...
                  labelFormatter={(value) => new Date(value as string | number | Date).toLocaleDateString()}
                />
                <Line 
                  type="monotone" 
                  dataKey="movements" 
        ...
        <Card className="glass-effect hover-glow">
          <CardHeader>
            <CardTitle className="text-glow">Storage Utilization</CardTitle>
