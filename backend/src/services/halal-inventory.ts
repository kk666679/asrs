...
export class HalalInventoryService {
  ...
  private static mockZones: HalalZone[] = [
    {
      ...
      isOperational: true,
      products: this.mockProducts.filter(
        (p) => p.storageZone === StorageZoneType.HALAL,
      ),
  ...
  static async getHalalStats(): Promise<HalalStats> {
    const totalHalal = this.mockProducts.filter(
      (p) => p.halalStatus === HalalStatus.CERTIFIED,
    ).length;
    const totalNonHalal = this.mockProducts.filter(
      (p) => p.halalStatus === HalalStatus.NON_HALAL,
    ).length;
    const expiringSoon = this.mockCertifications.filter(
      (c) => c.daysUntilExpiry <= 30,
    ).length;
    const nonCompliant = this.mockProducts.filter(
      (p) => p.halalStatus === HalalStatus.QUARANTINE,
    ).length;
    const totalOccupancy = this.mockZones.reduce(
      (sum, zone) => sum + zone.currentOccupancy,
      0,
    );
    const totalCapacity = this.mockZones.reduce(
      (sum, zone) => sum + zone.capacity,
      0,
    );
    const zoneOccupancy = Math.round((totalOccupancy / totalCapacity) * 100);
    const pendingVerification = this.mockProducts.filter(
      (p) => p.halalStatus === HalalStatus.PENDING,
    ).length;
