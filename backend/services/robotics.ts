...
export class RoboticsService {
  ...
  async getRobotStatus(robotId: string) {
    ...
    return {
      robot,
      queueLength: robot.robotCommands.length,
      estimatedIdleTime: this.calculateEstimatedIdleTime(robot.robotCommands),
    };
  }

  private calculateEstimatedIdleTime(robotCommands: any[]): number {
    return robotCommands.reduce((total, cmd) => {
