...
export class PerformanceService {
  ...
  private async calculateAccuracy(dateRange: { start: Date; end: Date }) {
    const [totalMovements, completedMovements, failedMovements] = await Promise.all([
    ...
    const accuracy = totalMovements > 0 ? (completedMovements / totalMovements) * 100 : 100;
    
    return {
      picking: Math.round(accuracy * 100) / 100,
      putaway: Math.round(accuracy * 100) / 100,
      inventory: Math.round((100 - (failedMovements / Math.max(totalMovements, 1)) * 100) * 100) / 100
    };
  }
  ...
  private async calculateEfficiency(dateRange: { start: Date; end: Date }) {
    ...
    const spaceUtilization = totalCapacity > 0 ? (totalLoad / totalCapacity) * 100 : 0;
    ...
    const robotUtilization = totalRobots > 0 ? (activeRobots / totalRobots) * 100 : 0;
    ...
    const days = Math.ceil((dateRange.end.getTime() - dateRange.start.getTime()) / (1000 * 60 * 60 * 24));
    const operatorProductivity = operatorCount > 0 ? (completedTasks / (operatorCount * days)) : 0;
  ...
  async generateReport(period: string = '30d') {
    ...
    return {
      ...
      summary: {
        ...
        averageAccuracy: (metrics.accuracy.picking + metrics.accuracy.putaway + metrics.accuracy.inventory) / 3,
        overallEfficiency: (metrics.efficiency.spaceUtilization + metrics.efficiency.robotUtilization + metrics.efficiency.operatorProductivity) / 3,
        totalCosts: metrics.costs.operationalCost + metrics.costs.maintenanceCost + metrics.costs.energyCost
      }
    };
  }
}